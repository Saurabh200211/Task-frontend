...
const AllTasks =() =>{
  ...
  useEffect(() =>{
          const fetch = async()  =>{
       const response =   await axios.get("https://task-backend-tan.vercel.app/v2/get-all-tasks",
             { headers }
          ); 
   ...
   <button onClick={() =>setInputDiv("fixed")}> <IoAddCircleSharp className="text-4xl text-gray-400 hover:text-gray-100 transition-all duration-300" />
   </button>
        </div>
       { Data &&(
